...
parser.add_argument(
    "--mindepth", type=float, default=10.0, help="Minimum of input depths"
)
parser.add_argument(
    "--maxdepth", type=float, default=1000.0, help="Maximum of input depths"
)
parser.add_argument(
    "--name",
    type=str,
    default="densedepth_nyu",
    help="A name to attach to the training session",
)
parser.add_argument(
    "--checkpoint", type=str, default="", help="Start training from an existing model."
)
parser.add_argument(
    "--full",
    dest="full",
...
#     with open(runPath + "/" + __file__, "w") as training_script:
#         training_script.write(training_script_content)

    # Generate model plot
    plot_model(
        model,
        to_file=runPath + "/model_plot.png",
        show_shapes=True,
